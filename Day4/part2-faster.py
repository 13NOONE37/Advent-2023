...
def get_cards(line_index,matches):
    ...
    if matches>0 and line_index+1+matches<len(lines):
        for i in range(line_index+1,line_index+1+matches):
            _, new_matches = strachcards_status[i]
            get_cards(i,new_matches)

for line in lines:
    line_index, matches = get_scratchcard_info(line)
    strachcards_status.append((line_index,matches))

for card in strachcards_status:
    get_cards(card[0],card[1])
